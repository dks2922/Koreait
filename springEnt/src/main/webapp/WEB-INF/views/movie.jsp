...
<div class="container">
		 <div align="center">
			...
			<c:if test="${contentboardList.currentPage > 1}">
				<input type="button" class="btn btn-default" value="맨앞" onclick="location.href='?subject=movie&currentPage=1'" title="첫 페이지 목록으로 이동하기"/>
			</c:if>
			<c:if test="${contentboardList.currentPage <= 1}">
			...
			<c:if test="${contentboardList.startPage > 1}">
				<input class="btn btn-default" type="button" value="이전" onclick="location.href='?subject=movie&currentPage=${contentboardList.startPage - 1}'" 
			...
			<c:forEach var="i" begin="${contentboardList.startPage}" end="${contentboardList.endPage}" step="1">
				...
				<c:if test="${contentboardList.currentPage != i}">
					<input class="btn btn-default" type="button" value="${i}" onclick="location.href='?subject=movie&currentPage=${i}'"
						title="${i}페이지로 이동합니다."/>
				</c:if>
			</c:forEach>
			
			<c:if test="${contentboardList.endPage < contentboardList.totalPage}">
				<input class="btn btn-default" type="button" value="다음" onclick="location.href='?subject=movie&currentPage=${contentboardList.endPage + 1}'" 
			...
			<c:if test="${contentboardList.endPage >= contentboardList.totalPage}">
 			...
 			<c:if test="${contentboardList.currentPage < contentboardList.totalPage}">
				<input class="btn btn-default" type="button" value="맨뒤" onclick="location.href='?subject=movie&currentPage=${contentboardList.totalPage}'" 
